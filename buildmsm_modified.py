...
#Modified by Guangfeng Zhou
#I only want the Populations.dat and Mapping.dat file to be written out.
#November,2,2012
...
def run(LagTime, Assignments, Symmetrize='MLE', Prior=0.0, OutDir="./Data/", OutFn=['Mapping.dat','Populations.dat']):
...
\nOnly output Populations.dat,Mapping.dat""")
    ...
    #parser.add_argument('-bs','--bootstrap',description='''If -bs is specified, the output file will be named automatically based on the Assignment#.h5 or Assignment#.Fixed.h5 file''',store_action='True')
    parser.add_argument('assignfilename')
    args = parser.parse_args()
    
    output_populations = os.path.join('/'.join(args.assignfilename.split('/')[:-1]),args.assignfilename.split('/')[-1].replace('Assignments','Populations').replace('h5','dat'))
    output_mapping = output_populations.replace('Populations','Mapping')
    OutFn = [output_mapping,output_populations]
    run(args.lagtime, args.assignments['Data'], args.symmetrize, args.prior,
        args.output_dir,OutFn)