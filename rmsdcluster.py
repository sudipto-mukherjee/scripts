...
for cutoff in cutoffs:
    ...
    if os.path.exists(targetDataDir):
        ...
        #run_cmd('Cluster.py -S 10 rmsd hybrid -d %3.3f'%(float(cutoff)/10))
        #run_cmd('Cluster.py -S 10 rmsd hybrid -d %3.3f'%(float(cutoff)/10),TESTING)
        #run_cmd('Assign.py rmsd')
        #run_cmd('Assign.py rmsd',TESTING)
        if BootStrap:
